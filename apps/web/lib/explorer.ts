function clusterQuerySuffix(): string {
  ...
  const isMain =
    cluster === "mainnet-beta" || cluster === "mainnet" || cluster === "mainnet-beta";
...
export function solanaAddressExplorerUrl(address: string): string {
  return `https://explorer.solana.com/address/${encodeURIComponent(address)}${clusterQuerySuffix()}`;
}

/** Solana Explorer URL for a transaction signature. */
export function solanaTxExplorerUrl(signature: string): string {
  return `https://explorer.solana.com/tx/${encodeURIComponent(signature)}${clusterQuerySuffix()}`;
}
